...
`init` / `update` 还会给其它编辑器写可选适配（见 [`ADOPT.md`](ADOPT.md)）；控制台派工只启动 **Cursor Agent** 或 **Claude Code**。
...
### 在浏览器里：Duaer-spec FDE（现场控制台）

本机开一个网页控制台（`duaer live`），专门管「从聊需求到派工、看进度、看结果」：

- **项目管理。** 建项目、切换项目；每个项目自己的对话和进度，互不搅在一起。
- **聊着把需求聊清楚。** 左边聊天；中间把目标、边界、验收写成确认卡；空话验收过不了，得能检查才给确认。发送旁可点 **录音**；在设置里配置 **语音识别模型**（兼容 OpenAI `/audio/transcriptions`，如 whisper-1）后，录完会解码成文字填入输入框，改完再发送。
...
- **看交付物和结果。** 可打开交付物页（需求时间线、架构、任务、交付记录）；做完了可打开页面或项目文件夹。
...
- **调度中心。** 独立页看派工图与各项目的系统架构，顶栏与控制台一致；长任务链会换行，节点显示实时状态。

### 数字员工：三种角色

控制台顶部点「数字员工」可看目录。运行时仍是 Cursor Agent 或 Claude Code；差别在**任务角色**——谁做实现、谁做回归、谁做部署。

| 角色 | 干什么 | 不干什么 |
|---|---|---|
| **实现员工** | 按验收写功能；更新产品 README；把预览跑起来并 stamp 交付 | — |
| **功能回归** | 对照确认卡里的验收标准，按产品仓 `.duaer/memory/testing.md` 做确定性回归（优先 Playwright、`npm test`、`npm run test:live` 等 L0–L3）；缺测就补测、跑通、留下证据 | **不改业务功能范围**；测挂了只留可复现说明，不擅自 stamp「验收通过」 |
| **部署员工** | 按你选的计划托管平台上线，把公网地址写入交付预览 | 不改业务功能范围 |

多人并行时：默认末位泳道承担**功能回归**（以及部署任务）；实现任务在前面的泳道。一个人时：同一 CLI 会话按任务角色依次做完。

控制台在放行前会自己跑产品仓 `.duaer/memory/verify.json` 里的命令。退出码不是 0，或没有这份契约，就不会保持「已交付」。纯文档任务要显式写 `{ "waiver": "docs-only" }`。

### 部署：支持的几种方式

开工前在控制台选 **计划托管平台**。选好后，数字员工按该平台约束写代码，并由**部署员工**负责上线（把公网地址写进交付预览）。

| 方式 | 适合做什么 | 你要准备什么 |
|---|---|---|
| **暂不部署** | 只在本地做完、先不发公网 | 无 |
| **GitHub Pages** | 静态站 / 文档站；未选平台但文案要上线时的默认 | GitHub 仓库与 `gh` 登录 |
| **Cloudflare** | 静态站用 Pages；接口 / 边缘逻辑用 Workers | 在控制台设置里保存 API Token 与 Account ID |
| **阿里云** | 静态多用 OSS（+ CDN）；接口可用函数计算等 | 在控制台设置里保存 AccessKey |
| **AWS** | 静态多用 S3 + CloudFront；也可用 Amplify / Lambda 等 | 在控制台设置里保存 Access Key |
...
```bash
# 启动（任意目录都行，不会写入当前项目；未配模型也会打开控制台）
duaer live
