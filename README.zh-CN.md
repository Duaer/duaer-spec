...
- **中 / 繁中 / 英 / 日 / 韩 / 西 / 葡 / 法 / 德 / 俄 / 越界面。** 台面文案可切换语种。
...
- **调度中心。** 独立页看派工图，顶栏与台面一致；长任务链会换行，节点显示实时状态。

### 数字员工：三种角色

台面顶部点「数字员工」可看目录。运行时仍是 Cursor Agent 或 Claude Code；差别在**任务角色**——谁做实现、谁做回归、谁做部署。
...
台面在放行前会自己跑产品仓 `.duaer/memory/verify.json` 里的命令。退出码不是 0，或没有这份契约，就不会保持「已交付」。纯文档任务要显式写 `{ "waiver": "docs-only" }`。
