...
| E2E-086 | After Confirm, architecture dialogue + Archify HTML under hosting (no diagram-container height clip in embed); gate dispatch; revise must re-confirm; ready chat says diagram is under hosting; Confirm architecture clickable after render | `npm test` + `npm run test:live` |
