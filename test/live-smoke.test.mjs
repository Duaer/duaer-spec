...
test("live L3 smoke: desk shell + validate gate", async (t) => {
  ...
  assert.match(html, /id="deliveryCockpit"|id="deliveryStages"|id="deliveryNextAction"/);
  assert.match(html, /id="workerCountList"|id="taskGraphMount"/);
  ...
  assert.match(html, /duaer\.live\.theme|data-theme/);
  ...
  assert.match(js, /persistProjectChat|loadProjectChatIntoUi|clearDeskWorkspace/);
  ...
  assert.match(html, /id="previewDeploy"/);
  assert.match(html, /id="deployPicker"|preview\.deployWhere/);
  assert.match(js, /openDeployPicker|startPreviewDeploy|deployPicker/);
  assert.match(js, /\/api\/deploy/);
  assert.match(liveBin, /\/api\/deploy|deployDispatchedJob/);
  ...
  assert.ok(
    (missingBody.issues || []).some((x) =>
      /设备矩阵|关键路径|异常态|接口契约|环境检查|数据预检|外部依赖|性能预算/.test(x),
    ),
  );
