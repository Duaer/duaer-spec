...
3. 每个功能模块维护确认卡字段：goal、outOfScope、acceptance、assumptions，以及 FDE-01 基线三块与 FDE-02 契约：
   - deviceMatrix = 浏览器/设备矩阵：至少两个具体目标（如 Chrome 最近两版、手机 Safari、统信 UOS）；禁止只写「主流浏览器」。必须写证据位置（截图/录屏或云测）和降级或 Polyfill。
   ...
   - perfBudget = 性能预算：LCP、INP、包体、长列表虚拟滚动、弱网、大数据压测；写「挺快的」不合格；无页面则写「本模块无页面性能要求」
...
2. 维护确认卡字段（仍用确认卡字段名，便于前端复用）：
   ...
   - perfBudget = 若有页面则更新 LCP、INP、包体、长列表虚拟滚动、弱网、大数据压测，或写「本模块无页面性能要求」/「同基线」
...
6. 输出格式（严格）：
   ...
   - 然后单独一行：<<<JSON>>>
   - 再输出 JSON（不要 markdown 围栏）：
{"goal":"...","outOfScope":"...","acceptance":"...","assumptions":"...","deviceMatrix":"...","criticalPaths":"...","exceptionCases":"...","apiContract":"...","envChecklist":"...","dataPrecheck":"...","externalDeps":"...","perfBudget":"...","ready":false,"options":["可选A","可选B"]}`;
...
const FIX_ACCEPT_PROMPT = `你是「Duaer-spec FDE」需求修正助手。自动验收未通过，请根据 issues 修订确认卡。优先把 acceptance 改成可客观检查的句子（打开何处、看到什么、哪条命令通过），并补全 deviceMatrix / criticalPaths / exceptionCases / apiContract / envChecklist / dataPrecheck / externalDeps / perfBudget；不要编造用户没提过的大功能。
...
function parseChatResult(content) {
  try {
      ...
      ? modulesRaw
          .map((m) => {
            ...
            return {
              ...
              perfBudget: flat(m.perfBudget ?? m.card?.perfBudget),
    ...
    return {
      ...
      perfBudget: flat(obj.perfBudget),
...
function chatDoneSsePayload(parsed, { includeJsonBlock = true } = {}) {
  return {
    ...
    perfBudget: String(parsed?.perfBudget || ""),
    ...
    modules: Array.isArray(parsed?.modules)
      ? parsed.modules.slice(0, 40).map((m) => ({
          ...
          perfBudget: String(m.perfBudget || "").slice(0, 4000),
          dependsOn: Array.isArray(m.dependsOn)
...
function localAcceptCheck(card, { kind = "feature" } = {}) {
  ...
  if (kind !== "bug") {
    ...
    if (deviceMatrix.length < 4 || missingCompatMatrix(deviceMatrix).length) {
      const missing = missingCompatMatrix(deviceMatrix);
      issues.push(
        missing.length
          ? `「浏览器/设备矩阵」未写成可核对兼容包：${missing.join("、")}`
          : "「浏览器/设备矩阵」必填（至少两个具体浏览器或国产终端，并写证据与降级）",
      );
    }
    ...
    if (!apiContractIsFilled(card.apiContract)) {
      issues.push(
        "「接口契约」必填（OpenAPI/类型路径，或写「本模块无 HTTP API」）",
      );
    }
    const envChecklist = String(card.envChecklist || "").trim();
    if (
      !envChecklistDeclaresNoCustomer(envChecklist) &&
      (envChecklist.length < 4 || missingEnvChecklist(envChecklist).length)
    ) {
      const missing = missingEnvChecklist(envChecklist);
      issues.push(
        missing.length
          ? `「环境检查」未通过：${missing.join("、")}（须探测 DNS、TLS、CORS、鉴权、第三方可达且全部通过，或写「无客户联调环境」）`
          : "「环境检查」必填（DNS、TLS、CORS、鉴权、第三方可达均通过，或写「无客户联调环境」）",
      );
    }
    const dataPrecheck = String(card.dataPrecheck || "").trim();
    if (
      !dataPrecheckDeclaresNoImport(dataPrecheck) &&
      (dataPrecheck.length < 4 || missingDataPrecheck(dataPrecheck).length)
    ) {
      const missing = missingDataPrecheck(dataPrecheck);
      issues.push(
        missing.length
          ? `「数据预检」未写清：${missing.join("、")}（须含字段映射、导入预检失败清单、可导出，或写「本模块无导入」）`
    ...
    const perfBudget = String(card.perfBudget || "").trim();
    if (
      !perfBudgetDeclaresNone(perfBudget) &&
      (perfBudget.length < 4 || missingPerfBudget(perfBudget).length)
    ) {
      const missing = missingPerfBudget(perfBudget);
      issues.push(
        missing.length
          ? `「性能预算」未写清：${missing.join("、")}（须含 LCP、INP、包体、长列表、弱网、大数据压测，或写「本模块无页面性能要求」）`
          : "「性能预算」必填（LCP、INP、包体、长列表、弱网、大数据压测，或写「本模块无页面性能要求」）",
      );
    }
  }
  return issues;
...
async function validateCardOnly(cfg, card, { kind = "feature", projectContext = null } = {}) {
  let normalized = {
    ...
    perfBudget: String(card.perfBudget || "").trim(),
  };
  ...
  return {
    ...
    perfBudget: review.perfBudget || normalized.perfBudget,
  };
}
...
async function autoFixConfirmCard(cfg, card, issues, { kind = "feature", projectContext = null } = {}) {
  ...
  return {
    ...
    perfBudget: String(obj.perfBudget || card.perfBudget || "").trim(),
  };
}
